...
(document.getElementById("searchNameBtn") as HTMLButtonElement).onclick = () => {
  const firstname = (document.getElementById("searchFirstName") as HTMLInputElement).value;
  const lastname = (document.getElementById("searchLastName") as HTMLInputElement).value;
  const results = manager.findStudentsByName(firstname, lastname);
...
(document.getElementById("searchEmailBtn") as HTMLButtonElement).onclick = () => {
  ...
  showList<Student>(results);
  alert(`พบEmail: ${results.length} คน`);
};
